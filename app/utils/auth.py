...
def create_and_store_refresh_token(db: Session, user) -> "RefreshToken":
    ...
    """
    from app.models.refresh_token import RefreshToken  # Lazy import to avoid circular refs

    raw_token = _generate_refresh_token()
    expires_at = _now() + timedelta(days=getattr(settings, "REFRESH_TOKEN_EXPIRE_DAYS", 30))
...
def rotate_refresh_token(db: Session, old_token_str: str, user) -> Optional["RefreshToken"]:
    from app.models.refresh_token import RefreshToken
...
def require_project_role(project_id_param: str, allowed_roles: list[str]):
    ...
    """
    def _dep(project_id: str = project_id_param, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
