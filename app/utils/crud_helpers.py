...
def mark_duplicate(db: Session, original_id: UUID, duplicate_id: UUID):
    ...
    log_activity(db, user_id=None, task_id=duplicate_id, action=f"Marked duplicate of {original.task_key}")
    clear_user_cache(original.project.owner_id if original.project else None)
    return {"detail": f"Task {duplicate.task_key} marked as duplicate of {original.task_key}"}
...
def log_activity(db: Session, user_id: UUID, task_id: UUID, action: str):
    ...
    log = models.ActivityLog(user_id=user_id, task_id=task_id, action=action, timestamp=datetime.utcnow())
    db.add(log)
    db.commit()
    db.refresh(log)
    if user_id:
...
def detect_possible_duplicates(db: Session, project_id: UUID, new_title: str, threshold: int = 80):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="RapidFuzz not installed. Run: pip install rapidfuzz")

    existing_tasks = db.query(models.Task).filter(models.Task.project_id == project_id).all()
